export default function Sidebar({ counts }) {
  // Define items with their corresponding dynamic values
  const sidebarItems = [
    ...
    { label: "Analytics", value: "" }, // No badge for analytics
        ...
        {sidebarItems.map((item) => (
          <li
            ...
            className={`flex justify-between items-center px-3 py-2 rounded cursor-pointer transition-colors ${
              item.label === "Posts"
                ? "bg-white text-black font-bold shadow-sm" // Highlight active section
                : "text-gray-600 hover:bg-indigo-50 hover:text-[#3b49df]"
            }`}
          >
            <span>{item.label}</span>
            {/* Only show badge if value is a number or non-empty string */}
            {item.value !== "" && (
